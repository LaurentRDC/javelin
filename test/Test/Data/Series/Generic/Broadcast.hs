...
testPropDivMaybe
    = testProperty "Broadcastable division with holes (/:)" $ property $ do
        m1 <- forAll $ Gen.map (Range.linear 0 100) ((,) <$> Gen.string (Range.singleton 2) Gen.alpha <*> Gen.double (Range.linearFrac (-500) 500))
        c  <- forAll $ Gen.double (Range.linearFrac (-500) 500)

        let (xs :: Series Vector String Double) = fromStrictMap m1
        forM_ (index xs) $ \k -> do
