...
class BroacdastFracMaybe a b c where
    (/:) :: a -> b -> c
...
class BroacdastFracMatched a b c where
    (/|) :: a -> b -> c
...
instance (Ord k, Fractional a) => BroacdastFracMaybe (Series k a) (Series k a) (Series k (Maybe a)) where
    (/:) :: Series k a -> Series k a -> Series k (Maybe a)
    (/:) = zipWith (/)
...
instance (Ord k, Fractional a) => BroacdastFracMatched (Series k a) (Series k a) (Series k a) where
    (/|) :: Series k a -> Series k a -> Series k a
    (/|) = zipWithMatched (/)
...
instance Fractional a => BroacdastFracMaybe (Series k a) a (Series k a) where
    (/:) :: Series k a -> a -> Series k a
    xs /: constant = fmap (/constant) xs
...
instance Fractional a => BroacdastFracMatched (Series k a) a (Series k a) where
    (/|) :: Series k a -> a -> Series k a
    xs /| constant = fmap (/constant) xs
...
instance Fractional a => BroacdastFracMaybe a (Series k a)  (Series k a) where
    (/:) :: a -> Series k a -> Series k a
    constant /: xs = fmap (constant/) xs
...
instance Fractional a => BroacdastFracMatched a (Series k a)  (Series k a) where
    (/|) :: a -> Series k a -> Series k a
    constant /| xs = fmap (constant/) xs